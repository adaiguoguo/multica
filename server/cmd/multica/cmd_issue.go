...
func init() {
	...
	issueCommentAddCmd.Flags().String("idempotency-key", "", "Stable key (unique per issue, max 128 chars) making this create safe to retry: a re-run with the same key returns the already-posted comment instead of duplicating it")
